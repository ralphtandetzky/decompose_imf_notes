...
Der Vorteil dieser Sichtweise ist, dass die Nebenbedingungen $a(t)>0$ wegen $a(t)=|\ee^{\sigma(t)}|$ automatisch erf"ullt ist. 
Es bleibt nur noch 
$$ \Im\dot\sigma(t)\ge0 $$ 
...
Die IMF ist der Realteil der Funktion $\ee^{\sigma(t)}$. 
...
$$ \sigma_i = \sum_{k=1}^{d_1}b_k\ee^{-\frac{(i-\mu_k)}{2\alpha^2}} + \ii\sum_{l=1}^{d_2}c_l\frac{1}{1+\ee^\frac{\nu_l-i}{\beta}} $$
